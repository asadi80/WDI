...
export default function WaferDetail() {
  ...
  const stats = useMemo(() => {
    ...
    const avgImages = data.defects.length > 0 ? (totalImages / data.defects.length).toFixed(1) : 0;

    return { edxCounts, avgSize, maxSize, totalDefects: data.defects.length, totalImages, avgImages };
  }, [data]);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">Loading CU wafer data...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div>
            ...
            >
              Back to Home
               ...
               <button
              onClick={() => router.push("/wafers")}
              className="text-sm text-gray-600 hover:text-orange-600 transition-colors mb-2 inline-flex items-center gap-1 ml-5"
            >
              Back to CU Wafers
            </button>
            <h1 className="text-3xl font-bold tracking-tight">
              CU Wafer {data.wafer.waferId}
            </h1>
          </div>
        ...
        <div className="grid grid-cols-5 gap-4">
          ...
          <div className="bg-orange-50 rounded-lg p-4">
            <div className="text-sm text-orange-600 font-medium">Total Images</div>
            <div className="text-2xl font-bold text-orange-900 mt-1">{stats.totalImages}</div>
          </div>
          <div className="bg-teal-50 rounded-lg p-4">
            <div className="text-sm text-teal-600 font-medium">Avg Images</div>
            <div className="text-2xl font-bold text-teal-900 mt-1">{stats.avgImages}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-lg">CU Wafer Map</h2>
            <select
              value={filterEDX}
              onChange={(e) => {
              ...
              className="px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none bg-white"
            >
          ...
          <div className="flex flex-col xl:flex-row gap-6 items-start">
            <div className="relative">
              <canvas
                ...
                width={500}
                height={500}
                className="rounded bg-white mx-auto block cursor-crosshair max-w-full"
                onClick={(e) => {
              ...
              {selectedDefect !== null && (
                <div className="absolute top-2 left-2 bg-white border border-gray-300 rounded-lg p-3 shadow-lg text-sm">
                  ...
                  <div className="space-y-1">
                    ...
                    <div>Images: {filteredDefects[selectedDefect].imageCount}</div>
                  </div>
                </div>
              )}
            </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 sticky top-0 border-b">
              <tr>
                ...
                >
                  <div className="flex items-center gap-2">
                    EDX Category {getSortIcon("edxCategory")}
                  </div>
                </th>
                <th
